function [wimage, support] = iat_inverse_warping(image, warp, transform, shift, nx, ny)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if shift
    xy(1,:)=xy(1,:)-length(nx)/2;
    xy(2,:)=xy(2,:)-length(ny)/2;
end
...
xy_prime = A * xy;
if shift
    xy_prime(1,:)=xy_prime(1,:)+length(nx)/2;
    xy_prime(2,:)=xy_prime(2,:)+length(ny)/2;
end
if strcmp(transform,'homography')
...
for i = 1:size(image,3)
    ...
    tempimage = interp2(image(:,:,i), xy_prime(1,:), xy_prime(2,:), 'linear');
    if i==1
